...
RVIZ_CONFIG = os.path.abspath(
    os.path.join(os.path.dirname(__file__), '..', 'rviz', 'localization_auto.rviz'))
...
def launch_setup(context, *args, **kwargs):
    ...
    start_x = float(context.perform_substitution(LaunchConfiguration('map_start_x')))
    start_y = float(context.perform_substitution(LaunchConfiguration('map_start_y')))
    start_yaw_deg = float(context.perform_substitution(LaunchConfiguration('map_start_yaw_deg')))
    lower_rad = math.radians(min_deg + offset)
    upper_rad = math.radians(max_deg + offset)
    laser_yaw_rad = math.radians(laser_yaw_deg)
    map_start_pose = [start_x, start_y, math.radians(start_yaw_deg)]
    ...
    slam_toolbox_node = LifecycleNode(
        ...
        output='screen',
        parameters=[
            os.path.join(CONFIG_DIR, 'slam_toolbox_localization_auto.yaml'),
            {
    ...
    activate_slam = RegisterEventHandler(
        OnStateTransition(
            ...
            goal_state='inactive',
            entities=[
                LogInfo(msg='[localization_auto] Activating slam_toolbox (auto initial pose)...'),
                EmitEvent(
                    event=ChangeState(
                        lifecycle_node_matcher=matches_action(slam_toolbox_node),
    ...
    rviz_after_map = TimerAction(
        period=rviz_delay,
        actions=[
            LogInfo(msg='[localization_auto] Opening RViz (static map on /map)...'),
            rviz_node,
        ],
    )
...
def generate_launch_description():
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'map_start_x',
            default_value='0.0',
            description='Assumed start X (m) in the map frame (0 = where mapping began).',
        ),
        DeclareLaunchArgument(
            'map_start_y',
            default_value='0.0',
            description='Assumed start Y (m) in the map frame (0 = where mapping began).',
        ),
        DeclareLaunchArgument(
            'map_start_yaw_deg',
            default_value='90.0',
            description='Assumed start heading (deg) in the map frame. +90 points map +Y '
                        '(left in the top-down RViz view). Set this to the way you are facing.',
        ),
